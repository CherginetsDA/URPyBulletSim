...
def main():
    ...
    while t < 200:
        ...
        pose = robot.fk_pose(robot.position, quaternion = False)
        print(pose[:3])
        qwe = [pose[2], 0.15 + 0.1*np.sin(t), 0.015 + 0.1*np.sin(t) - pose[2]]
        # sp.update(pose.flatten().tolist()[:3])
        sp.update(qwe)
        ddq = robot.impedance_control_signals(
            np.array([ 0.5, 0.2, 0.15 + 0.1*np.sin(t)] + rot),
            np.array([0]*2 + [0.1*np.cos(t)] + [0]*3),
            np.array([0]*2 + [-0.1*np.sin(t)] + [0]*3),
            fd = True
        )
        # print(ddq)
        dv = 5e-3*(acc +  ddq.flatten())
        # print(dv)
        temp_vel = robot.velocity + dv
        acc = ddq.flatten()
        dp = 5e-3*(robot.velocity + temp_vel)
        # print(dp)
        new_position = robot.position + dp
        # print(new_position)
        robot.set_state(state = new_position, value = .5)
        robot.update_joint_states()
